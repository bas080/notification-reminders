...
- **Process-Level Crash Handling**: `NotificationRemindersApplication` registers a global `Thread.setDefaultUncaughtExceptionHandler` at process initialization. Uncaught JVM exceptions, non-fatal errors, and ANR events are intercepted by a global error handler at runtime and logged to `SharedPreferences`. Stack traces are retrieved directly with line numbers, class names, and exception types included in the generated report text. On next app startup, `MainActivity` prompts the user to send a crash report email to `bas080@hotmail.com`.
