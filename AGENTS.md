...
- **Process-Level Crash Handling**: `NotificationRemindersApplication` registers a global `Thread.setDefaultUncaughtExceptionHandler` at process initialization. Uncaught exceptions from any thread or component are logged to `SharedPreferences`. On next app startup or feedback request, a dedicated screen (`CrashReportActivity`) presents a multi-line text input for the user to describe what they were doing when the issue occurred. All collected data, breadcrumbs, metadata, stack traces, and user notes are formatted into a structured, human-readable Markdown string (`CrashReportActivity.buildFormattedReport`) before sending a report email to `bas080@hotmail.com`.
